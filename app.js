...
function generatePattern() {
  ...
  for (const pixel of sampled) {
    ...
    const nearest = findNearestMardColor(lab);
    cells.push(nearest);
  }
...
function showCellEditor(row, col) {
  ...
  const replacementLabels = [originalColor.label, ...(originalColor.alternatives || [])];
  for (const label of replacementLabels) {
    const replacement = mardByLabel.get(label);
...
function findNearestMardColor(lab) {
  let nearest = mardColors[0];
  let bestDelta = Number.POSITIVE_INFINITY;
  for (const color of mardColors) {
    const delta = deltaE2000(lab, color.lab);
    if (delta < bestDelta) {
      bestDelta = delta;
      nearest = color;
    }
...
function layoutLegend(palette, cell) {
  const itemWidth = Math.max(70, cell * 3.6);
  const itemHeight = 68;
...
function drawLegend(ctx, palette, layout, x0, y0, canvasWidth) {
  ...
  palette.forEach((color, index) => {
    ...
    ctx.fillText(`替代:${(color.alternatives || []).join("/")}`, x + swatchW / 2, y + swatchH + 30);
  });
}
